...
А во-вторых, "сглаживание" не работает, если изменения тока или напряжения нет. Разрешение в таком случае "родное", 11-разрядное. Что сводит к нулю выгодоприобретение в виде 16-разрядного результата. Как сказал доктор - шансов нет, но попробовать можно.

В итоге: для ПИД-регулирования параметры сглаживания не должны вность излишнюю задержку, а потому значения 3-4 оптимальны (устанавливаются в редиме Device). Для отображения браузером и дисплеем параметры сглаживания могут задаваться разработчиком соответствуя желанию при реализации каждого состояния процесса, что не влияет на ПИД-регулирование. 
