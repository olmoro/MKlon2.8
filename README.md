...
#### <p align="right">редакция от 20 марта 2023 года
...
MKlon2. Как это работает. Интересное наблюдение.

Часовой прогон в режиме самопрогрева.

Ранее было замечено, что с прогревом прибора показания тока и напряжения становятся нулевыми по истечении какого-то времени, и с этим никто не будет спорить. И вот подвернулся случай - коммунальная авария - в квартире похолодало до +24°C. При закрытом корпусе температура по датчику на радиаторе устанавливается около 29°C - значит 5 градусов в нашем распоряжении. Запасся терпением, и вот результат:
...
Следует учесть, во-первых, что "родным" для АЦП является 11-разрядный результат измерения. Для напряжения это будет примерно 20000/2048 = 10мВ на единицу. Для регистрации медленно меняющихся токов и напряжений имеется возможность путем математической обработки повысить разрешение до одного милливольта и миллиампера, подобрав соответствующий параметр "сглаживания". Нулевое значение параметра будет соответствовать отсутствию "сглаживающего" эффекта и быстрой реакции на изменение тока и напряжения и наоборот, с увеличением параметра система будет реагировать всё медленнее. Нет особого смысла увеличивать сглаживание (он же усреднение, он же низкочастотный фильтр) до максимального значения, равного 5, но для калибровок измерителей это самое то.   

А во-вторых, "сглаживание" не работает, если изменения тока или напряжения нет. Разрешение в таком случае "родное", 11-разрядное. Что сводит к нулю выгодоприобретение в виде 16-разрядного результата. И третий знак после зпт демонстрировать вовсе не обязательно. Как сказал доктор - шансов нет, но попробовать можно.

В итоге: для ПИД-регулирования параметры сглаживания не должны вность излишнюю задержку, а потому значения 3-4 оптимальны (устанавливаются в редиме Device). Для отображения браузером и дисплеем параметры сглаживания могут задаваться разработчиком как и число отображаемых знаков после запятой  соответствуя желанию при реализации каждого состояния процесса, что никак не влияет на ПИД-регулирование. 
...
"Вы полагаете, все это будет носиться? — Я полагаю, что все это следует шить." (Ю.Левитанский)
